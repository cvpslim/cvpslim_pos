   <?php
   //print_r($mesas);
   $area = $mesas[0]['tmp_mesas']['area_id'];
   
   
    foreach ($mesas as $mesa):
        if($mesa['tmp_mesas']['area_id']!=$area){
            $area = $mesa['tmp_mesas']['area_id'];
            echo '<div class="alert nopadding col-lg-12 col-md-12 col-sm-12 col-xs-12"></div>';
        }
