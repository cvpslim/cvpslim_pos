...
echo $this->Form->input('credito',array('label' =>__('Credito'),'type' => 'text','disabled' => 'disabled'));
?>
    <div>
        <div id="boton_agregar">
            <button class="btn btn-warning" onclick="agregarCredito('<?php echo $this->data['Cliente']['id']; ?>'); return false">Agregar credito</button>
        </div>
        <div id="form_oculto"></div>
    </div>
<?php
echo $this->Form->input('porcentaje_descuento', array('type' => 'text'));
echo $this->Form->end(array('class'=>'btn btn-success', 'label'=>'Guardar registro'));
?>
</div>
