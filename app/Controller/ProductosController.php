...
public function index1() {
 ...
 public function add() {
   ...
   if (!empty($this->data)) {
			...
			if ($this->Producto->save()) {
				$this->Session->setFlash(__('<p class="bg-primary">Registro agregado exitosamente. Para continuar  agregue  la informacion obligatoria en la parte derecha de la pantalla.</p>'));
                ...
                $clases=$this->Producto->Clase->find('list',array('fields'=>'Clase.nombre_clase' ));
 ...
 public function consubcategoria($id_sub=null) {
     if (!empty($this->data)) {
			$this->Producto->create($this->data);
			if ($this->Producto->save()) {
				$this->Session->setFlash('Registro agregado exitosamente');
...
function edit($id = null) {
    ...
    $img=$this->Producto->query("SELECT productos.imagen
                                    FROM productos
                                    WHERE productos.id='$id'");
...
//                     exit();
       ...
       public function delete($id = null) {
           ...
           $this->Producto->query("DELETE FROM prodinfs WHERE prodinfs.producto_id='$id'");
		if (!$id) {
...
//                     exit();
        ...
        public function listarporcategoria($id=null) {
            ...
            $this->Producto->recursive = 1; 
             $productos=$this->Producto->query("SELECT categorias.nombre as categoria ,categorias.id
                                                FROM categorias
                                               ");
    
            $this->set('productos', $productos);
            
            $ps=$this->Producto->query("SELECT productos.id
                                                FROM productos
                                                where productos.id='$id'
                                               ");
            ...
            if($id!=null){
                ...
                $listars=$this->Producto->query("SELECT categorias.nombre, subcategorias.nombre,productos.nombre,productos.precio,productos.descripcion,productos.id
                                                    FROM productos,subcategorias,categorias
                                                    where subcategorias.id=productos.subcategoria_id and categorias.id=subcategorias.categoria_id  and categorias.id='$id'

                                               ");
              
               $this->set('listars', $listars);
          ...
          public function listarporcategoria2($id=null) {
...
public function buscador() {
       ...
       $prods=$this->Producto->query("SELECT DISTINCT productos.id,productos.nombre,productos.precio,productos.descripcion,productos.imagen,productos.subcategoria_id,productos.colore_id,productos.fuente_id,productos.estado
                                        FROM productos
                                        where productos.nombre  like '_%$texto%' or productos.nombre  like '$texto%' 
...
public function  alfa($a=NULL) {
            
               $this->Producto->recursive=0; 
                         $datos=$this->Producto->query("SELECT productos.nombre,productos.imagen,productos.precio,productos.id
                                                        from productos
