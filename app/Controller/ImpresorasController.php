...
class ImpresorasController extends AppController{
    ...
    public function index() {
     $this->Impresora->recursive=1;
     $impresoras = $this->Impresora->find('all');
     
     //print_r($impresoras);
     $this->set('impresoras', $impresoras);
    }

    public function add() {
		if (!empty($this->data)) {
			$this->Impresora->create($this->data);
			if ($this->Impresora->save()) {
    ...
    function edit($id = null) {
		...
		if (!empty($this->data)) {
			if ($this->Impresora->save($this->data)) {
		...
		if (empty($this->data)) {
			$this->data = $this->Impresora->read(null, $id);
		}
    }
}
