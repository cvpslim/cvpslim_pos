...
class UsersController extends AppController
{
  ...
  public function login() 
  {
    if ($this->request->is('post')) 
    {
      ...
      {
        $this->Session->setFlash(__('Usuário o contraseña inválidos, intente nuevamente'),'flash_fail');
      }
    }
  }
  ...
  public function register()
  {
    if ($this->request->is('post')) 
    {
      ...
      {
        $this->Session->setFlash(__('Usuario guardado com éxito'),'flash_success');
  ...
  public function edit($id = null) 
  {
    ...
    if ($this->request->is('post') || $this->request->is('put')) 
    {
      ...
      {
        $this->Session->setFlash(__('Imposible guardar usuário. Intente nuevamente.'),'flash_fail');
      }
    } 
    else 
    {
  ...
  public function delete($id = null) 
  {
    ...
    if ($this->User->delete()) 
    {
      $this->Session->setFlash(__('Usuário eliminado'),'flash_success');
      $this->redirect(array('action' => 'home'));
    }

    $this->Session->setFlash(__('Imposíble eliminar usuário'),'flash_fail');
    ...
    if( $this->request->is('post') )
    {
      ...
      if( $this->request->data['User']['password'] === $this->request->data['User']['re_password'] )
      {
        ...
        else # Maybe hes comming from change password form
        {
          ...
          {
            throw new MethodNotAllowedException(__('Accion inválida'));
          }
        }
  ...
  */
  public function remember_password()
  {
    if( $this->request->is('post') )
    {
      ...
      if( empty($user) )
      {
        $this->Session->setFlash('el email no existe en nuestra base de datos.','flash_fail');
      ...
      ->config('gmail')
      ->emailFormat('html')
      ->subject(__('Recordar clave - '.Configure::read('Application.name')))
      ->to( $user['User']['email'] )
      ->from( Configure::read('Application.from_email') )
      ->viewVars(array('hash' => $hash))
      ->send();        

      $this->Session->setFlash('Revisa tu e-mail para para continuar el proceso de recuperacion de clave.','flash_success');
